...
`iNEXT.UniFrac` uses the observed sample of OTU count data (called the "reference sample") 
...
<td align="left">A numerical vector specifying the particular value of sample coverage (between 0 and 1). <code>level = 1</code> means complete coverage  \cr
...
The `iNEXTUniFrac()` function returns the `"iNEXTUniFrac"` object including two data frames for each regions: 
...
The `ggiNEXTUniFrac()` function is a wrapper around the `ggplot2` package to create a R/E curve using a single line of code. The resulting object is of class `"ggplot"`, so it can be manipulated using the `ggplot2` tools.
